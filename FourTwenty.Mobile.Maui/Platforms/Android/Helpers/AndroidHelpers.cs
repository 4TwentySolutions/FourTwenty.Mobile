...
namespace FourTwenty.Mobile.Maui.Helpers
{
    public static class AndroidHelpers
    {
        ...
        [SupportedOSPlatformGuard("android")] // note that this option only added in .NET 7.0
        public static bool IsAndroidAtLeast(BuildVersionCodes version) => OperatingSystem.IsAndroidVersionAtLeast((int)version);

        public static void SetShiftMode(this BottomNavigationView bottomNavigationView, bool enableItemShiftMode)
        {
            try
            {
                if (bottomNavigationView.GetChildAt(0) is not BottomNavigationMenuView menuView)
                {
        ...
        public static void TintView(View view, int color)
        {
            ...
            nd?.SetColorFilter(AppCompatDrawableManager.GetPorterDuffColorFilter(color, PorterDuff.Mode.SrcIn));
        ...
        public static void SetCursorDrawableColor(EditText editText, Color color)
        {
            try
            {
                if (IsAndroidAtLeast(BuildVersionCodes.Q))
                {
